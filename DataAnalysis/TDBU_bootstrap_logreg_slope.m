...
function bootstrapTDBU = TDBU_bootstrap_logreg_slope(dataA, dataL, Day, itt, replacement)
...
parfor t=1:itt
    ...
    output = TDBU_scale_logreg_slope(sh_data_A, sh_data_L,Day); % calculate TD, BU indices
...
real = TDBU_scale_logreg_slope(dataA, dataL, Day);
...
% prepare output
bootstrapTDBU = struct('ciTD',ciTD,'ciBU',ciBU,'medTD',medTD, 'medBU',medBU,'real',real, 'resTD',resTD,'resBU',resBU, 'Pval', Pval);
